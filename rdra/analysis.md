...
- 管理者
  - 派遣元会社の社長が想定ユーザー。経理担当がいない小規模企業が対象のため、細かなロール分割は一旦しない
...
外部の情報源として、協会けんぽの都道府県別料率と国税庁の税額表がある。V1ではシステム連携せず、料率・税額表をマスタや定数として取り込む。マイナンバーの取り扱いはファーストスコープ外。
...
| 勤怠管理 | 打刻の収集（LINE打刻・勤務時間入力・画像・CSV/Excel）、5区分への振り分け、例外修正、承認 | 従業員・管理者 |
...
- 勤怠管理 — 打刻という「事実」を集める範囲。従業員がLINEで打刻・勤務時間入力・画像送信し、管理者がCSV/Excelも含めて取り込み・修正・承認する。事業所マスタと突合して基本・残業・朝残業・深夜・休日の5区分へ自動で振り分ける
...
**変更の詳細**: 日次打刻に、その日の実打刻時間（始業・終業）が事実として記録される。自然言語の場合はシステムが時刻に変換する。勤務時間の直接入力にも対応し、打刻ボタン・自然言語・時間入力の3経路を持つ。丸めはせず1分単位で残す。どの事業所での打刻かもあわせて持つ。
...
**変更の詳細**: 管理者が日次打刻の時刻や休憩時間を手で上書きする。上書きした行は黄色くハイライトされ、手修正された事実が残る。休憩を0分や30分に減らすと、その分が労働時間として計算される。
...
**変更の詳細**: 従業員の承認要求で日次打刻が承認待ちになる。管理者の承認で承認済みになり、集計へ進める。差し戻すと記録済みに戻り、再度修正・確認する。
