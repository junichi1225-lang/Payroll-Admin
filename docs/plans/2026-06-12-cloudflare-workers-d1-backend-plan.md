...
マルチテナント設計（モックの `tenantId` を踏まえた単一 DB か、テナントごとの DB 分割か）は論点として 7 章に残します。初期実装は単一 DB + `tenant_id` カラムで進めます。

### 3.3 契約駆動開発: OpenAPI を単一の合意点にする
...
OpenAPI 仕様から Orval がフロントエンド用クライアントとサーバー用バリデーションを生成し、フロントエンドは生成クライアント経由で Workers 上の API を呼び、Workers が D1 に永続化する構成です。給与計算ロジックは共有ライブラリとして Workers から使い、API への切替が完了するまでの間はフロントエンドからも参照します。
...
計算ロジックの抽出（3.4）と API クライアントへの切替（5.5）は payroll-app に手を入れざるを得ません。この 2 種類の変更に限り、以下の手順を踏みます。
...
1. **認証方式** — 誰がこのシステムを使いますか。社内の労務担当者のみなら Cloudflare Access で十分です。従業員本人が給与明細を見るポータルを兼ねるなら、ユーザー認証（メール + パスコード、または外部 IdP）の設計が必要で、工数が大きく変わります
2. **マルチテナントの粒度** — モックには `tenantId` が存在します。複数企業へ提供する SaaS を想定しているなら、テナントごとに D1 データベースを分ける選択肢（情報漏えいリスクの構造的低減）も検討の余地があります。社内利用のみなら単一 DB で十分です
3. **Replit でのモック公開の継続要否** — バックエンド接続後も Replit 上のプレビューを PdM の確認環境として使い続けるか。続ける場合、Replit から preview 環境の Workers API を参照する CORS 設定が必要です
4. **個人情報の取り扱い範囲** — マイナンバー等の要配慮情報を将来扱う予定があるか。あるなら暗号化方針（カラムレベル暗号化等）を Phase 1 のスキーマ設計に反映する必要があります
5. **localStorage 上の既存データの移行要否** — モックで PdM が入力したデータを実 DB に引き継ぐか。検証用データであれば移行せず、シードスクリプトを用意する方が安全です
...
1. 本計画書のレビュー（特に 7 章の論点 1・2 は Phase 3 までに決着が必要です）
2. 承認後、Phase 0 の実装計画を作成して着手します
